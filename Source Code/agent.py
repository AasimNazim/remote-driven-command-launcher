...
def get_office_apps_running():
    ...
    try:
                    ...
                    if proc_name_lower in ['winword.exe', 'powerpnt.exe']:
                        ...
                        try:
                            ...
                            if hwnds:  # Process has visible windows
                                has_window = True
                                print(f"📋 Found Office app with window: {proc_name} (PID: {proc.pid})")
                            else:
                                print(f"🚫 Skipping Office process without window: {proc_name} (PID: {proc.pid})")

                        except ImportError:
                            # If win32gui not available, fall back to basic check
                            has_window = True
                            print(f"📋 Found Office app (no window check): {proc_name} (PID: {proc.pid})")
                        except Exception as e:
                            print(f"⚠️ Window check failed for {proc_name}: {e}")
        ...
        print(f"❌ Error checking office apps: {e}")
...
def show_shutdown_popup():
    ...
    try:
        ...
        print("⚠️ Shutdown warning: Please close Word and PowerPoint applications.")
...
def wait_for_office_apps_close():
    ...
    while True:
        ...
        if not running_office_apps:
            print("✅ Word and PowerPoint applications closed.")
            shutdown_system()
            return True

        print(f"⏳ Waiting for Office apps to close: {running_office_apps}")
        time.sleep(2)


def shutdown_system():
    """Shutdown the system."""
    print("🔴 Shutting down system in 10 seconds...")
    time.sleep(10)
    try:
...
def read_sms():
    try:
        ...
        # print("ADB OUTPUT (first 500 chars):\n", output[:500], "\n")
    ...
    while True:
        ...
        if  sms_text == "open word" :
                ...
                if not is_sms_recent(sms_date):
                    print("❌ SMS is not recent. Skipping command.")
                    time.sleep(5)
                    continue
                else:                
                    print("📌 Command detected: open_word")
                    try:
                        process = subprocess.Popen([WORD_PATH])
                        executing_processes["open word"] = process
                        print("📌 Command executed: open_word")
                    except Exception as e:
                        print(f"❌ Error executing open word: {e}")
                ...
                if not is_sms_recent(sms_date):
                    ...
                    print("📌 Command detected: open_ppt")
                    try:
                        process = subprocess.Popen([POWERPOINT_PATH])
                        executing_processes["open ppt"] = process
                        print("📌 Command executed: open_ppt")
                    except Exception as e:
                        print(f"❌ Error executing open ppt: {e}")
                ...
                if not is_sms_recent(sms_date):
                    ...
                    if running_office_apps:
                        print(f"📋 Office apps running: {running_office_apps}")
                        # Show popup to close Office applications
                        show_shutdown_popup()
                        # Wait for Office applications to close
                        wait_for_office_apps_close()
                    else:
                        print("✅ No Office applications running. Proceeding with shutdown.")
                    # Shutdown system
                    shutdown_system()
                ...
                if not is_sms_recent(sms_date):
                    ...
                    print("🔴 Combined command detected")
                    # Open PowerPoint
                    try:
                ...
                if not is_sms_recent(sms_date):
                    ...
                    if "open word" in executing_processes:
                        ...
                        print("📌 Command executed: closed_word")
                    else:
                        print("❌ No running process found for closed_word")
                ...
                if not is_sms_recent(sms_date):
                    ...
                    if "open ppt" in executing_processes:
                        ...
                        print("📌 Command executed: closed_ppt")
                    else:
                        print("❌ No running process found for closed_ppt")
        time.sleep(5)
